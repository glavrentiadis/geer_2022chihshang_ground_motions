function [d v a, d_c, v_c, a_c, varargout]=baselineK(t,ai,method,varargin)
...
if method==2
    %Evaluate af baseline af (t2 to end of record)
    i=find(t>=t1);
    G=[t(i).^2 t(i) ones(size(vi(i)))];  %Inversion Kernel
...
if method==4
    ...
    af=m(2);
    %Find time
    kfit=find(t<=t2+0.001 & t>=t2-0.001);
    t2fit=(af*tf^2+2*v0*tf-v0*t1+2*(K-di(kfit)))/(v0+af*t1);
