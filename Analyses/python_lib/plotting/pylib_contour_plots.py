...
def PlotScatterCAMap(scat_latlondata, cmin=None,  cmax=None, flag_grid=False, title=None, cbar_label=None, log_cbar = False, 
    ...
    cs =  ax.scatter(x_scat, y_scat, s = data_scat_s, c = data_scat_c, 
                     transform = data_crs, vmin=cmin, vmax=cmax, zorder=3, alpha = alpha_v)
...
def PlotMap(lat_lims = None, lon_lims = None, flag_grid=False, title=None):
    ...
    #background
    # fname_bkg = '../../../Raw_files/cartopy/NE1_HR_LC_SR_W_DR/NE1_HR_LC_SR_W_DR.tif'
    # fname_bkg = '../../../Raw_files/cartopy/NE1_50M_SR_W/NE1_50M_SR_W.tif'
    # ax.imshow(imread(fname_bkg), origin='upper', transform=data_crs, 
    #           extent=[-180, 180, -90, 90])
    fname_bkg = '../../../Raw_files/cartopy/noaa_etopo_bedrock_15arcsec.tiff'
    # ax.imshow(imread(fname_bkg), origin='upper', transform=data_crs, cmap=plt.gray(),
    #           extent=[119.50, 122.50, 21.50, 25.50])
    image_bkg = Image.open(fname_bkg).convert("L")
    arr_bkg   = np.asarray(image_bkg)
    ax.imshow(arr_bkg, origin='upper', transform=data_crs,  cmap='gray', vmin=0, vmax=255*0.5,
               extent=[119.50, 122.50, 21.50, 25.50])
    
    #add land zones
    lands = cfeature.LAND
    ax.add_feature(lands, zorder=1, alpha=0.70)
    #add costal lines
    ...
    oceans = cfeature.NaturalEarthFeature(category='physical', name='ocean', facecolor='lightblue',
                                          scale=plt_scale)
    ax.add_feature(oceans, zorder=2, alpha=0.90)
